...
// ─── requireLogin — redirects to index.html if not authenticated ─────────────
// Uses a short grace period to let Firebase resolve auth state.
// If still not logged in after the grace period, redirects to index with a
// return URL so the user is sent back after signing in.
function requireLogin() {
  const page = location.pathname.split('/').pop() || 'index.html';
  if (page === 'index.html' || page === '') return; // never gate the home page

  const GRACE_MS = 3000; // ms to wait for Firebase auth to resolve
  let resolved = false;

  // If Firebase resolves quickly with a logged-in user, do nothing
  // If it resolves with no user, or times out, redirect to login
  const checkTimer = setTimeout(() => {
    if (!resolved && !getCurrentUser()) {
      _redirectToLogin();
    }
  }, GRACE_MS);

  // Hook into auth state — called by onAuthStateChanged
  const prev = window._requireLoginCallback;
  window._requireLoginCallback = (user) => {
    resolved = true;
...
function _redirectToLogin() {
  ...
  location.href = `index.html?r=${returnUrl}`;
}
...
async function initFirebase() {
  ...
  _auth.onAuthStateChanged(async user => {
    ...
    // Notify requireLogin watcher
    if (typeof window._requireLoginCallback === 'function') {
      window._requireLoginCallback(user);
    }
    // Notify index.html auth callback
    if (typeof window._onAuthChangedCallback === 'function') {
      window._onAuthChangedCallback(user);
    }
